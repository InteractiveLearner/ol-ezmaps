...
JSON files are a way to transmit information to a web application. For the sake of this web application, the JSON file is used to send information locally so that the map container can be filled as desired. Users open up the JSON and enter in the details required for their visualization. You can see in the image below what an empty ```application.json``` file looks like. Examples are provided if you want to get started quickly. 

![Empty JSON](img/emptyJSON.png)

###### View

The view is where you want the map to be centred. The coordinates for the view are in the form of longitude and latitude.

###### Layers

Each layer you wish to add to the map container will have to be stored in the ```layers``` object. The ```layer``` object should be renamed to whatever you plan on titling the layer as. 
...
**Note:** You can have a ```proportional``` symbol map that is also a ```choropleth``` or ```identified``` map. All depends on whether you want 1 or 2 themes for your feature layer. Just remember you can't have 3 themes set at the same time in one layer. 
